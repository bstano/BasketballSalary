...
The VIF Feature set gives us the best performing Regression Tree
...
cv <- function(t, r, s = 1) {
  ...
  for (i in 1:r) {
    ...
    new.tree.vif <- tree(log_salary ~ ., bdata.vif[train,])
    pred.tree <- predict(new.tree.vif, newdata = bdata.vif[-train,]) 
    vif.test <- bdata.vif[-train, "log_salary"]
    tree.cv <- mean(c(((pred.tree - vif.test)^2)[[1]]))
