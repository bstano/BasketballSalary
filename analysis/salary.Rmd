...
```{r}
ols.log2 <- glm(log_salary ~ ., data = bdata.log2)
...
```{r}
ols.log3 <- glm(log_salary ~ ., data = bdata.log3)
...
```{r}
ols.log4 <- glm(log_salary ~ ., data = bdata.log4)
...
```{r}
ols.log5 <- glm(log_salary ~ ., data = bdata.log5)
...
```{r}
fit.large <- glm(log_salary ~ ., data = bdata.log)

fit.small <- glm(log_salary ~ 1, data = bdata.log)
...
vars.step
map(vars.step, function(.) . %in% vars.vif)
